...
file_dir = "/Users/wesleychang/OneDrive - Drexel University/Chang Lab/General/Group/Data/Ultrasound/"
...
files = ['20240212_LMFP-SiO2_y30_z40_del16_dur6_2']
...
# This function incorporates a cross-correlation (convolution integral) to find the wave time-of-flight shift (difference in transmission times between two waves)
...
def plot_images(amp):
    ...
    gs = plt.GridSpec(rows,cols+1)
    ax1 = fig1.add_subplot(gs[:,0])
    
    x_dim = 80+1  #should be set to the appropriate image scan size
    y_dim = 60+1
    vmin = 1 #set the vmin and vmax to change the color intensities 
    vmax = 8
...
for n,f in enumerate(files):
    connection = sqlite3.connect(file_dir+f+'.sqlite3')
    ...
    delay = 16
    duration = 6
    tofs = np.linspace(delay, delay+duration,len(waves[0]))
    ...
    # ax1.plot(freqs,amps[0], color = scalar_map.to_rgba(n))
    # ax1.set_xlim(0,100E6)

    for wave in waves[17:]:
    #     ax1.plot(wave, color = c)
    #     break
    ...
    plot_images(amps)
    # ax1.plot(times, amps/amps[0], color = c) # plots the normalized amplitude intensity relative to the first waveform collected 
    # ax2.plot(times, tofshift, color = c) 
...
# ax1.set_ylabel('Amplitude Norm. [A]/[A$\mathregular{_0}$]')
# ax2.set_ylabel('Time of flight shift ($\mathregular{\mu}$s)')

# for ax in [ax1, ax2]:
#     ax.set_xlim(0, 20)
# ax2.set_ylim(-0.05,0.05)
plt.savefig('20240212_LMFP_LiFun_cell.tiff')
plt.show()